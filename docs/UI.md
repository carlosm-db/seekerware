...
| `/blocks` · `/blocks/template-check` | **Bank** | v4 LinkedIn-inspired (2026-07-18): roles are **collapsed cards, newest first** (title · company · dates; migration 0009) — tap to open; one **pencil per item** edits in place (server-rendered editor state via `?edit/?editrole/?add/?addrole`); role setup = Title/Company/From/To months + "I currently work here" checkbox + Code last; **saving IS the approval** (migration 0010; no draft/approve cycle, no pills, no separate edit page); EN+ES both always required; bullets and roles **hard-delete with confirm** (owner decision — role delete removes its bullets too); **Check template** diffs the bank against the Doc's `{{…}}` tokens both ways | 2 |
